...
data        = np.load(ROOT / 'trajectory_data_nonlinear.npy', allow_pickle=True).item()
params      = data['params']
rB          = params['rB']
t_sim       = params['T']
l_path_scale = params['l_path_scale']
all_results = data.get('all_results', None)

# Define l_fun to match example_nonlinear.py
def l_fun(t):
    lam = max(1 - t / t_sim, 0.0)
    return np.array([-l_path_scale * lam, l_path_scale * lam])
...
# Compute l_fun(t) on demand using definition from example_nonlinear.py
l_t = np.array([l_fun(t) for t in all_results[0]['tgrid']])

ax1.plot(l_t[:, 0], l_t[:, 1],
         style['l_t_ls'], linewidth=style['l_t_lw'],
         color='black', label=r'$\mathrm{Safe\ path\ from}\ x_r\to 0$')

# Mark reference point x_r at the first point of l_t
ax1.plot(l_t[0, 0], l_t[0, 1], marker='D', markersize=7,
         color='purple', linestyle='None', label=r'$x_r$')

ax1.plot(0, 0, '*', markersize=style['origin_sz'], color='blue')
...
fig1.tight_layout()
fig1.savefig(figures_dir / 'trajectories_nonlinear.png', dpi=600, bbox_inches='tight')
plt.close(fig1)
print("[Saved] Figures/trajectories_nonlinear.png")
...
fig2.tight_layout()
fig2.savefig(figures_dir / 'levelsets_hU_nonlinear.png', dpi=600, bbox_inches='tight')
plt.close(fig2)
print("[Saved] Figures/levelsets_hU_nonlinear.png")
...
fig3.tight_layout()
fig3.savefig(figures_dir / 'barrier_ball_nonlinear.png', dpi=600, bbox_inches='tight')
plt.close(fig3)
print("[Saved] Figures/barrier_ball_nonlinear.png")
...
fig4.tight_layout()
fig4.savefig(figures_dir / 'barrier_safety_nonlinear.png', dpi=600, bbox_inches='tight')
plt.close(fig4)
print("[Saved] Figures/barrier_safety_nonlinear.png")
...
fig5.tight_layout()
fig5.savefig(figures_dir / 'control_nonlinear.png', dpi=600, bbox_inches='tight')
plt.close(fig5)
print("[Saved] Figures/control_nonlinear.png")

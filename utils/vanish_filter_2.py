...
class VanishingFilter:
    ...
    def solve_qp(
        ...
        # Ensure Gurobi uses the local license file unless user overrides it
        os.environ.setdefault('GRB_LICENSE_FILE', '/Users/andreasoliveira/gurobi.lic')

        try:
            model = gp.Model('vanishing_cbf_lp')
            model.Params.OutputFlag = 0
            model.Params.FeasibilityTol = max(1e-9, min(1e-2, eps))
            model.Params.OptimalityTol = max(1e-9, min(1e-2, eps))

            k_vars = model.addVars(m, lb=-GRB.INFINITY, ub=GRB.INFINITY, name='k')

            # CBF constraints: A k <= b
            for i in range(A.shape[0]):
                model.addConstr(
                    gp.quicksum(float(A[i, j]) * k_vars[j] for j in range(m)) <= float(b[i]),
                    name=f'cbf_{i}'
                )
            ...
            if self.u_norm_type == 'inf':
                for j in range(m):
                    model.addConstr(k_vars[j] <= self.u_max, name=f'u_inf_ub_{j}')
                    model.addConstr(-k_vars[j] <= self.u_max, name=f'u_inf_lb_{j}')
            elif self.u_norm_type == 'l1':
                abs_k = model.addVars(m, lb=0.0, name='abs_k')
                for j in range(m):
                    model.addConstr(abs_k[j] >= k_vars[j], name=f'abs_pos_{j}')
                    model.addConstr(abs_k[j] >= -k_vars[j], name=f'abs_neg_{j}')
                model.addConstr(gp.quicksum(abs_k[j] for j in range(m)) <= self.u_max, name='u_l1')
            else:
                raise ValueError(f"Unknown norm type: {self.u_norm_type}")

            # Objective: maximize <f(x) + l(t), g(x,t)k>
            g_xt = self.g(x, t)
            drift = self.f(x) + l_t
            c_eff = g_xt.T @ drift
            model.setObjective(
                gp.quicksum(float(c_eff[j]) * k_vars[j] for j in range(m)),
                GRB.MAXIMIZE
            )
            ...
            if model.Status == GRB.OPTIMAL:
                return np.array([k_vars[j].X for j in range(m)])

            if model.Status == GRB.INFEASIBLE:
                print(f"Warning: LP infeasible at t={t:.4f} (Gurobi status=INFEASIBLE)")
                print(f"  b1={b1:.4f}, b2={b2:.4f}, h_B={h_B:.4f}, h_U={h_U:.4f}")
                print(f"  ||L_g_h_U||={np.linalg.norm(L_g_h_U):.4f}, u_max={self.u_max}")
            else:
                print(f"Warning: Gurobi did not return OPTIMAL at t={t:.4f}. Status={model.Status}")

            return k0

        except gp.GurobiError as exc:
            raise RuntimeError(
                "Gurobi solve failed. Ensure gurobipy is installed and license is valid at "
                "/Users/andreasoliveira/gurobi.lic"
            ) from exc
